...
const Header = () => {
  ...
  return (
    <header
      className={`top-0 z-40 mx-auto w-full flex-none bg-primary-200 transition-all duration-100 ease-in dark:bg-dark-900 md:backdrop-blur-sm dark:md:bg-dark-900/90 ${
    ...
    >
      <div className="mx-auto w-full max-w-7xl py-3 px-3 md:flex md:justify-between md:py-3.5 md:px-4">
        ...
        <nav
          ...
          } w-auto overflow-y-auto dark:text-dark-200 md:mx-5 md:flex md:h-auto md:items-center md:overflow-visible`}
              ...
              links.map(({ label, href, icon: Icon, links }, index) => (
                <li key={`item-link-${index}`} className={links?.length ? 'dropdown' : ''}>
                  {links && links.length ? (
                    <>
                      <button
                        className="flex items-center px-4 py-3 font-medium transition duration-150 ease-in-out hover:text-dark-900 dark:hover:text-tertiary-100"
                      ...
                      <ul
                        ...
                        } rounded pl-4 font-medium drop-shadow-xl md:absolute md:min-w-[200px] md:bg-tertiary-100/90 md:pl-0 md:backdrop-blur-md dark:md:bg-dark-900/90`}
                      >
                        {links.map(({ label: label2, href: href2 }, index2) => (
                          <li key={`item-link-${index2}`}>
                            <Link
                              className="whitespace-no-wrap block py-2 px-5 first:rounded-t last:rounded-b dark:hover:bg-dark-700 md:hover:bg-dark-200"
